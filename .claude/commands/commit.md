...
2. **Full test suite** — If no code files are staged (same definition as step 1), skip tests with a note. Otherwise run `pytest -x -q`. All tests must pass. If any fail, diagnose the root cause, fix it, then re-run. Repeat until all pass in a single run.
...
5. **Recommend commit message** — Analyze `git diff --cached` and the staged file list to generate a recommended message:
   ...
   - Present the recommended message and ask the user to confirm, edit, or reject. Wait for explicit confirmation before proceeding.
