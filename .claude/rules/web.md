...
- `POST /api/v2/models/image-classifier/retrain` — kicks off `scripts/retrain_image_triage.py` as a detached subprocess via `_spawn_retrain_runner` (mirrors `src/web/routes/ingest.py::_spawn_ingest_runner`). Returns `202 + {run_id, status: 'running'}`. Three server-side gates, each returning a distinct error shape:
  ...
  - **Concurrency**: `EXISTS (SELECT 1 FROM model_training_runs WHERE model_type='image_relevance' AND status='running')` → 409 `{error: "retrain_already_running", running_run_id}`. Multi-worker gunicorn could otherwise fire two simultaneous retrains.
