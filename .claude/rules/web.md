...
**Legacy backfill actionable queue.** The banner count is wrapped in an `<a href="?legacy_backfill=1">` so reviewers can click through to a filtered filings list. The `?legacy_backfill=1` query param on `/v2/review/filings` activates the legacy-backfill filter — a virtual queue that restricts the list to filings with `images_legacy_pending > 0`.
