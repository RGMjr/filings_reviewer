...
`/v2/review/stats` **Images tab** opens with five cards backed by `db.get_image_decision_breakdown_v2()` (single roundtrip). The Summary-tab Image Confirmations card uses the independent `db.get_image_decision_overall_v2()` (Relevant / Not Relevant per-metric decision rollup) plus `db.get_image_review_progress_v2()` (per-image rollup) — the three helpers stay decoupled so the simpler Summary view is unaffected by changes to the per-decision breakdown.

The image-level tiles in the Summary card — Pending Images / Reviewed Images / Not Relevant Images (with sub-counts Skipped / Rejected) — are sourced from `v_analytics_image_review_progress` (`sql/202605191650_*.sql`). The view mirrors `_derive_image_review_state`: it counts images whose per-metric flow is complete with ≥1 accept/correct/add as "reviewed" rather than reading the raw `v2_image_assets.review_status` column (which the per-metric flow leaves at `'pending'` by design). Legacy `review_status='reviewed'` is also honoured as "reviewed" so historical V1 reviewer activity stays counted. The Not-Relevant split distinguishes the "Skip image" button path from the "Reject all — no relevant metrics" sentinel path (sentinel row: `decision='reject'`, `rejection_reason='no_relevant_metrics'`, NULL metric ids). See `docs/operations/analytics-ui-runbook.md` for the column-level contract.
