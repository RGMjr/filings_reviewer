...
**`file_path` and `predicted_relevance` survive a NULL inbound:** two columns on `v2_image_assets` use `COALESCE(EXCLUDED.<col>, v2_image_assets.<col>)` in the upsert clause; every other column overwrites unconditionally.
- **`file_path`** (legacy-103, 2026-04-27): a force-reextract whose SEC fetch failed (transient outage, malformed URL) preserves the existing R2 storage key.
- **`predicted_relevance`** (gh-478, 2026-05-04): a re-extraction with `USE_LEARNED_TRIAGE=false` does not set the score on the in-memory asset; COALESCE preserves the previously-cached score rather than nulling it. `ImageTriageStage` is the normal writer when the gate is on; `scripts/score_image_candidates.py` is the offline backfill writer for legacy NULL rows.
...
- **Financial-line-item row guard (`value_binding.py`).** `_FINANCIAL_LINE_ITEM_STUB_RE` matches financial-statement row labels (accounts payable/receivable, margin, EBITDA, liabilities, etc.); `_FINANCIAL_LINE_ITEM_STUB_ALLOW` = `{cm_gross_margin_by_cohort, cm_revenue_concentration}` exempts the two metrics that legitimately bind such rows. Two enforcement points: (1) **column-scan** (`iterate_rows=True`) skips per-cell when the row's `stub_path` is a financial line item — guards all non-allow-listed metrics; (2) **row-scan** (`iterate_rows=False`) + **Strategy 5** (candidate cell holds the value) call `_is_financial_line_item_count_row`, which fires only for `_COUNT_ONLY_METRICS` (`unit_compatibility.py`). The row-scan guard closes the FP where a count keyword spuriously matches a substring of a line-item label (e.g. `cm_customers_period_end`'s `\btotal\s+accounts?\b` matching the stub "Total accounts receivable", then row-binding AR dollar balances as customer counts). It is scoped to count metrics because financial metrics like `cm_gross_margin_overall` legitimately row-bind from margin rows.
