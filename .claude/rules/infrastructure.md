...
- `filings-onboarding-runner` — background worker (`src.universe.onboarding_runner --watch`); drains both `v2_ingest_batches` and the `model_training_runs` retrain queue (gh-400, retrain rows are prioritized so UI-triggered retrains don't wait behind a long onboarding batch). **Important (gh-469):** this service calls `process_filing()` inline during ingest, so it needs the same image-pipeline flags as `filings-extraction` (`USE_LEARNED_TRIAGE`, `LEARNED_TRIAGE_MIN`, `ENABLE_METRIC_CLASSIFY`, `VISION_CLASSIFY_*`, `GOOGLE_API_KEY`). Always mirror these flags on both services when adding new extraction-pipeline env vars.
...
| `ENABLE_METRIC_CLASSIFY` | Optional | Feature gate for the Vision-API metric-classify stage (`src/extraction_v2/stages/image_classify.py`). Default off; **set to `true` on both `filings-extraction` and `filings-onboarding-runner` in prod as of 2026-05-04 (gh-441 / gh-469)** — dev default remains off. When `true`, every chart / table_image runs `VisionClient.analyze_image_for_metric_classification` and the result lands in `v2_image_classifications`. Both services must be kept in sync — the onboarding worker calls `process_filing()` inline during ingest. |
...
| `GOOGLE_API_KEY` | With `VISION_CLASSIFY_PROVIDER=gemini` | Google AI Studio key for Gemini vision calls. **Note:** `src/llm/providers/gemini.py` reads `GOOGLE_API_KEY` (google-genai SDK convention), NOT `GEMINI_API_KEY`. Set manually in Render dashboard (`sync: false`) on both `filings-extraction` and `filings-onboarding-runner` (gh-469). Not in a shared env group — must be entered per-service in the Render UI. |
| `USE_LEARNED_TRIAGE` | Optional | Feature gate for the learned image-relevance triage in `src/extraction_v2/stages/image_triage.py`. Default `false`; **set to `true` on both `filings-extraction` and `filings-onboarding-runner` in prod as of 2026-05-04 (gh-442 / gh-469)**. When on, `predict_relevance()` is called per-image and `v2_image_assets.predicted_relevance` is populated; images scoring below `LEARNED_TRIAGE_MIN` are dropped from OCR/Vision. Reads model via the R2 pointer at `models/image_relevance/latest_run_id.txt`. Both services must be kept in sync. |
| `LEARNED_TRIAGE_MIN` | With `USE_LEARNED_TRIAGE=true` | Minimum model score required for an image to pass the gate. Default `0.4`; **prod uses `0.32`** on both `filings-extraction` and `filings-onboarding-runner` (gh-442 / gh-469) — slightly below the current `cba5e60f` model's ~80%-recall threshold (0.341). |
...
| `SENTRY_DSN` | Optional | Sentry error-monitoring DSN. When set, `src/infra/sentry.py::init_sentry()` wires error capture into the web app, the onboarding worker, and the extraction cron (errors-only — `traces_sample_rate=0`; PII-scrubbed — `send_default_pii=False` + an extended denylist). No-op when unset or under `APP_ENV=testing`. Lives in the `filings-shared-secrets` env group so it reaches exactly `filings-reviewer`, `filings-extraction`, and `filings-onboarding-runner` — not `filings-nightly-sweep` (bash/Claude CLI) or `filings-metabase`. **Free Developer tier** = 5K events/mo, 1 dashboard user, 14-day retention, overages silently dropped; multi-person triage needs the paid Team plan. The `LoggingIntegration` captures `logger.error` as events (the default) — the #1 quota tuning knob if a handled-error logger gets noisy (`ignore_logger("<name>")` or raise `event_level`). |
