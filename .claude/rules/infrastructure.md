...
| `GEMINI_API_KEY` | With `VISION_CLASSIFY_PROVIDER=gemini` | Google AI Studio key for Gemini vision calls. Set manually in Render dashboard (`sync: false`) on both `filings-extraction` and `filings-onboarding-runner` (gh-469). Not in a shared env group — must be entered per-service in the Render UI. |
