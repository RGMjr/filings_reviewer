...
class TestDatabaseIntegration:
    ...
    def test_get_filing_content_no_file_returns_none(self, tmp_path):
        """No file on disk -> returns None (no DB fallback)."""
        no_db_fetcher = FilingFetcher(storage_root=str(tmp_path / "test_filings"))

        content = no_db_fetcher.get_filing_content("0001234567", "0001234567-12-123456")

        assert content is None

    def test_get_filing_content_prefers_file_over_db(self, fetcher):
        """File exists -> returned directly, no DB query made."""
        cik = "0001234567"
        accession = "0001234567-12-123456"
        storage_dir = fetcher._get_storage_dir(cik, accession)
        storage_dir.mkdir(parents=True, exist_ok=True)
        ...
        assert content is not None
        fetcher.db.query.assert_not_called()
