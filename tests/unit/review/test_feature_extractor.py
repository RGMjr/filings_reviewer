...
class TestExceptionHandling:
    """Tests for exception handlers in defensive code paths."""

    def test_value_magnitude_with_overflow(self):
        """Should handle OverflowError in log10 calculation gracefully."""
        from unittest.mock import Mock
        import math

        extractor = FeatureExtractor()

        # Create a mock Decimal that causes OverflowError on log10
        mock_value = Mock(spec=Decimal)
        mock_value.__eq__ = lambda self, other: False  # Not None, not 0

        # Make float(mock_value) work but cause overflow in log10
        def mock_float():
            # Return a value that causes log10 to overflow
            raise OverflowError("float too large to convert")

        mock_value.__float__ = mock_float

        # Call internal method directly to test exception handler
        result = extractor._compute_value_magnitude(mock_value)
        # Should return None for magnitude due to overflow
        assert result is None

    def test_value_magnitude_with_type_error(self):
        """Should handle TypeError in value magnitude calculation gracefully."""
        extractor = FeatureExtractor()

        # Create a mock Decimal that raises TypeError on float conversion
        from unittest.mock import Mock

        mock_value = Mock(spec=Decimal)
        mock_value.__eq__ = lambda self, other: False  # Not None, not 0
        mock_value.__float__ = Mock(side_effect=TypeError("Cannot convert"))

        # Call internal method directly to test exception handler
        result = extractor._compute_value_magnitude(mock_value)
        assert result is None

    def test_definition_patterns_with_malformed_context(self):
        """Should handle exceptions in definition pattern matching gracefully."""
        from unittest.mock import Mock, patch

        extractor = FeatureExtractor()

        # Mock a pattern that raises TypeError during search
        with patch("src.review.feature_extractor.DEFINITION_PATTERNS") as mock_patterns:
            mock_pattern = Mock()
            mock_pattern.search.side_effect = TypeError("Pattern error")
            mock_patterns.__iter__ = lambda self: iter([mock_pattern])

            result = extractor._check_definition_language("test context")
            # Should return False instead of raising exception
            assert result is False

    def test_period_patterns_with_malformed_context(self):
        """Should handle exceptions in period pattern matching gracefully."""
        from unittest.mock import Mock, patch

        extractor = FeatureExtractor()

        # Mock a pattern that raises AttributeError during search
        with patch("src.review.feature_extractor.PERIOD_PATTERNS") as mock_patterns:
            mock_pattern = Mock()
            mock_pattern.search.side_effect = AttributeError("Pattern error")
            mock_patterns.__iter__ = lambda self: iter([mock_pattern])

            result = extractor._check_period_mention("test context")
            # Should return False instead of raising exception
            assert result is False

    def test_risk_factors_patterns_with_malformed_context(self):
        """Should handle exceptions in risk factors pattern matching gracefully."""
        from unittest.mock import Mock, patch

        extractor = FeatureExtractor()

        # Mock a pattern that raises TypeError during search
        with patch("src.review.feature_extractor.RISK_FACTORS_PATTERNS") as mock_patterns:
            mock_pattern = Mock()
            mock_pattern.search.side_effect = TypeError("Pattern error")
            mock_patterns.__iter__ = lambda self: iter([mock_pattern])

            result = extractor._check_risk_factors(
                context_text="test context",
                section_heading=None,
                section_path=None,
            )
    ...
    def test_context_word_count_attribute_error(self):
        """Should handle AttributeError in context word counting gracefully."""

        # Create a str subclass that raises AttributeError on split()
        class BrokenString(str):
            def split(self, *args, **kwargs):
                raise AttributeError("split() is broken")

        extractor = FeatureExtractor()
        weird_context = BrokenString("Test context")

        # This tests the exception handler at lines 152-153
        features = extractor.compute_features(
            number_value=Decimal("100"),
            number_unit=None,
            number_raw_text="100",
            keyword_distance=10,
            keyword_position="after",
            context_text=weird_context,  # This will cause split() to fail
        )
        # Should default to 0 on AttributeError
        assert features.context_word_count == 0
