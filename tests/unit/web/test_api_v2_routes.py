...
# Reusable UUID constants for test data
TEST_FACT_ID = "aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee"
TEST_DECISION_ID = "11111111-2222-3333-4444-555555555555"
TEST_NEXT_FACT_ID = "ffffffff-eeee-dddd-cccc-bbbbbbbbbbbb"
TEST_EXISTING_DECISION_ID = "99999999-8888-7777-6666-555555555555"
...
class TestCreateDecision:
    ...
    def test_create_accept_decision_success(self, client, mock_db):
        ...
        mock_db.insert_v2_review_decision.return_value = TEST_DECISION_ID
        ...
        assert data["decision_id"] == TEST_DECISION_ID
        assert data["fact_id"] == TEST_FACT_ID
    ...
    def test_missing_decision_returns_400(self, client, mock_db):
        ...
        with patch("src.web.routes.api_v2.get_db", return_value=mock_db):
            response = client.post(
                "/api/v2/decisions", json={"fact_id": TEST_FACT_ID}
            )
    ...
    def test_invalid_decision_type_returns_400(self, client, mock_db):
        ...
        with patch("src.web.routes.api_v2.get_db", return_value=mock_db):
            response = client.post(
                "/api/v2/decisions",
                json={"fact_id": TEST_FACT_ID, "decision": "approve"},  # not a valid V2 decision type
            )
    ...
    def test_fact_not_found_returns_404(self, client, mock_db):
        ...
        with patch("src.web.routes.api_v2.get_db", return_value=mock_db):
            response = client.post(
                "/api/v2/decisions",
                json={"fact_id": TEST_FACT_ID, "decision": "accept"},
            )
    ...
    def test_duplicate_decision_returns_409_via_existing_decision_id(self, client, mock_db):
        ...
        mock_db.get_v2_fact_by_id.return_value = {
            "fact_id": TEST_FACT_ID,
            "doc_id": 42,
            "decision_id": TEST_EXISTING_DECISION_ID,
        }
        ...
        data = json.loads(response.data)
        assert data["status"] == "error"
        assert data["existing_decision_id"] == str(TEST_EXISTING_DECISION_ID)
    ...
    def test_next_fact_included_in_response(self, client, mock_db):
        ...
        assert data["next_fact"]["fact_id"] == str(TEST_NEXT_FACT_ID)
        assert data["next_fact"]["url"] == f"/v2/review/{filing_id}?fact_id={TEST_NEXT_FACT_ID}"
...
class TestUndoDecision:
    ...
    def test_undo_decision_success(self, client, mock_db):
        ...
        with patch("src.web.routes.api_v2.get_db", return_value=mock_db):
            response = client.delete(f"/api/v2/decisions/{TEST_DECISION_ID}")
        ...
        mock_db.delete_v2_review_decision.assert_called_once_with(TEST_DECISION_ID)
