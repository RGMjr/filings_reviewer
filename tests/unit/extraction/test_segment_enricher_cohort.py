...
class TestRetentionMetricPatterns:
    ...
    def test_net_dollar_retention_rate_with_percent(
        ...
        result = enricher._detect_cohort_breakdowns(segment.raw_text, segment.candidate_metric_ids, None)
    ...
    def test_gross_retention_rate(self, enricher: SegmentEnricher) -> None:
        ...
        segment = _make_segment("Our gross retention rate of 95% demonstrates customer satisfaction.")
        result = enricher._detect_cohort_breakdowns(segment.raw_text, segment.candidate_metric_ids, None)
        assert result is True
...
class TestFiscalYearCohortPatterns:
    ...
    def test_the_year_cohort(self, enricher: SegmentEnricher) -> None:
        ...
        segment = _make_segment("The 2019 cohort represents customers acquired in that fiscal year.")
        result = enricher._detect_cohort_breakdowns(segment.raw_text, segment.candidate_metric_ids, None)
        assert result is True
    ...
    def test_fiscal_year_without_cohort_is_not_match(
        self, enricher: SegmentEnricher
    ) -> None:
...
class TestArrCohortPatterns:
    ...
    def test_annual_recurring_revenue_by_cohort(self, enricher: SegmentEnricher) -> None:
        ...
        segment = _make_segment(
            "Annual recurring revenue by cohort shows strong net expansion."
        )
        result = enricher._detect_cohort_breakdowns(segment.raw_text, segment.candidate_metric_ids, None)
        assert result is True
...
class TestEnterpriseCustomerPatterns:
    ...
    def test_lowercase_paid_customers_is_not_match(
        self, enricher: SegmentEnricher
    ) -> None:
    ...
    def test_expansion_within_paid_customer_base(
        self, enricher: SegmentEnricher
    ) -> None:
        """Matches 'expansion within our Paid Customer base'."""
        segment = _make_segment(
            "We measure the rate of expansion within our Paid Customer base."
        )
        result = enricher._detect_cohort_breakdowns(segment.raw_text, segment.candidate_metric_ids, None)
        assert result is True
...
class TestNegativeCases:
    ...
    def test_arr_in_unrelated_context_no_match(self, enricher: SegmentEnricher) -> None:
        ...
        segment = _make_segment("Our subscription arrangements generally have annual contractual terms.")
        result = enricher._detect_cohort_breakdowns(segment.raw_text, segment.candidate_metric_ids, None)
        assert result is False

    def test_fiscal_year_financial_reporting_no_match(
        self, enricher: SegmentEnricher
    ) -> None:
...
class TestRealS1Snippets:
    ...
    def test_slack_fiscal_year_cohort_definition(
        self, enricher: SegmentEnricher
    ) -> None:
...
class TestEdgeCasesAndVariations:
    ...
    def test_very_long_text_with_pattern(self, enricher: SegmentEnricher) -> None:
        ...
        segment = _make_segment(
            f"{prefix}Our Net Dollar Retention Rate was 143%.{suffix}"
        )
        result = enricher._detect_cohort_breakdowns(segment.raw_text, segment.candidate_metric_ids, None)
        assert result is True
...
class TestCoverageEdgeCases:
    ...
    def test_multiple_cohort_metric_ids_triggers_detection(
        self, enricher: SegmentEnricher
    ) -> None:
    ...
    def test_single_cohort_metric_id_does_not_trigger(
        self, enricher: SegmentEnricher
    ) -> None:
