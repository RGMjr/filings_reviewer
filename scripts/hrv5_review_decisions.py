...
NOTE: This script operates on V1 tables (review_candidates, review_decisions).
V2 facts use a different model with built-in confidence scoring.
Automated V2 decisioning is a separate future project.
...
def analyze_filing(db, filing_id: int, dry_run: bool = False) -> dict:
    ...
    for candidate in candidates:
        ...
        if is_fp:
            ...
            stats["rejected"] += 1
            stats["fp_patterns"][rejection_reason] = (
                stats["fp_patterns"].get(rejection_reason, 0) + 1
            )
