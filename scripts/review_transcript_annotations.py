#!/usr/bin/env python3
"""
Terminal-Based Transcript Annotation Review Tool.

Presents each LLM pre-annotated candidate one at a time with raw text context.
Reviewer assigns a disposition code to each candidate, then has the option to
add any metrics the LLM missed.
...
Trap detection: 2-3 synthetic false positives are injected by preannotate_transcript.py.
...
Output:
  data/transcript_gold_standard/{TICKER}_{DATE}_reviewed.csv

Usage:
    python3 scripts/review_transcript_annotations.py \\
        data/transcript_gold_standard/CRM_2025-02-26_preannotated.csv

    # Review multiple files back-to-back
    python3 scripts/review_transcript_annotations.py \\
        data/transcript_gold_standard/CRM_2025-02-26_preannotated.csv \\
        data/transcript_gold_standard/META_2025-04-30_preannotated.csv

    # Resume a partial review session
    python3 scripts/review_transcript_annotations.py \\
        data/transcript_gold_standard/CRM_2025-02-26_preannotated.csv --resume
"""
...
def find_transcript_text(ticker: str, date: str) -> str | None:
    """
    Find the raw transcript .txt file for context display.
    Handles date suffixes like CRM_2019-12-03_17_00_00.txt.
    """
    for candidate in TRANSCRIPT_DIR.glob(f"{ticker}_{date}*.txt"):
        try:
...
def find_context_in_transcript(raw_text: str, transcript: str | None, window: int = 400) -> str:
    """Find surrounding context for a raw_text snippet in the full transcript."""
    if not transcript or not raw_text:
        return ""

    # Find the snippet (first 60 chars as anchor)
    anchor = raw_text[:60].strip()
    idx = transcript.find(anchor)
    if idx == -1:
        return ""

    start = max(0, idx - window)
    end = min(len(transcript), idx + len(raw_text) + window)
...
def review_candidate(
    ...
    clear_screen()
    print(bold(f"\n{'='*65}"))
    print(bold(f"  TRANSCRIPT ANNOTATION REVIEW  [{index}/{total}]"))
    print(bold(f"{'='*65}"))
    print(
    ...
    print()
    print(cyan("  Raw text from transcript:"))
    wrapped = textwrap.fill(raw_text, width=62, initial_indent="    ", subsequent_indent="    ")
    print(wrapped)
    print()

    # Show surrounding context if available
    context = find_context_in_transcript(raw_text, transcript)
    if context and context.strip() != raw_text.strip():
        print(cyan("  Surrounding context:"))
        ctx_wrapped = textwrap.fill(
            context.strip(), width=62, initial_indent="    ", subsequent_indent="    "
        )
        print(ctx_wrapped)
        print()
...
def _collect_edits(candidate: dict) -> dict:
    ...
    # source_type
    current_st = candidate.get("source_type", "text")
...
def collect_add_annotations(ticker: str, date: str, company: str) -> list[dict]:
    """
    After all LLM candidates are reviewed, prompt reviewer to add any missed items.
    """
    added = []
    print()
    print(bold(f"{'='*65}"))
    ...
    print(bold(f"{'='*65}"))
    print("  Enter any metrics the LLM missed. Type 'done' to finish.")
    print()
    ...
    while True:
        ...
        source_type = prompt_optional("  source_type [text]: ", default="text")
        raw_text = prompt_optional("  raw_text (verbatim from transcript): ")
        ...
        row = {
            ...
            "is_trap": "false",
            "disposition": "ACCEPT",
            "disposition_reason": "Added by reviewer (LLM missed)",
        }
        added.append(row)
...
def print_session_stats(stats: dict) -> None:
    ...
    if stats["manually_added"] == 0:
        ...
        print(yellow("  Consider whether the LLM missed any metrics."))
...
def review_file(input_path: Path, resume: bool) -> None:
    ...
    # Load transcript for context display
    transcript = find_transcript_text(ticker, date)
    if transcript:
        print(f"Loaded transcript context ({len(transcript):,} chars)")
    else:
        print(yellow(f"No transcript text found for {ticker}_{date} — context unavailable"))
    ...
    for idx, candidate in enumerate(pending, start=1):
        result = review_candidate(candidate, idx, total, transcript)
...
def main():
    parser = argparse.ArgumentParser(
        description="Terminal-based transcript annotation review tool.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=textwrap.dedent(
            """
            Examples:
              %(prog)s data/transcript_gold_standard/CRM_2025-02-26_preannotated.csv
              %(prog)s data/transcript_gold_standard/*.csv --resume
        """
        ),
    )
    parser.add_argument(
