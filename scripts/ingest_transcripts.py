#!/usr/bin/env python3
"""
Batch Transcript Ingestion — Fetch and process earnings call transcripts.

Fetches transcripts from a document source (FMP or HuggingFace), runs them
through the V2 pipeline, and optionally persists results to the database.

Usage:
    # Dry run — show what would be ingested (no DB writes)
    python3 scripts/ingest_transcripts.py --source fmp --tickers CRM ADBE --dry-run

    # Persist to database
    python3 scripts/ingest_transcripts.py --source fmp --tickers CRM ADBE

    # Limit transcripts per ticker
    python3 scripts/ingest_transcripts.py --source fmp --tickers MSFT --limit 3

    # HuggingFace source
    python3 scripts/ingest_transcripts.py --source huggingface --tickers CRM

Environment variables:
    DATABASE_URL     PostgreSQL connection string (required for --persist)
    FMP_API_KEY      FMP API key (required when --source fmp)
"""
...
@dataclass
class IngestResult:
    """Result for a single transcript."""
    source_id: str
    ticker: str
...
class IngestSummary:
    ...
    def print_report(self) -> None:
        print()
        print("=" * 70)
        print("TRANSCRIPT INGESTION SUMMARY")
        print("=" * 70)
        ...
        if self.results:
            ...
            for r in self.results:
                ...
                if r.status == "ingested":
                    print(
                        f"    {r.ticker:6s}  {period:4s}  {date_str}  "
                        f"{r.fact_count} facts  filing_id={r.filing_id}"
                    )
                elif r.status == "skipped":
                    print(f"    {r.ticker:6s}  {period:4s}  {date_str}  [skipped — already ingested]")
                else:
                    print(f"    {r.ticker:6s}  {period:4s}  {date_str}  [ERROR: {r.error}]")
        print()
...
def _get_or_create_company(conn: Any, ticker: str, company_name: str) -> int:
...
def _upsert_transcript_filing(
    ...
    """
    Create or update a filings record for a transcript.

    Uses (ticker, document_date) as the unique key for transcripts.
    Returns the filing_id.
    """
    # Synthetic accession_number scoped to transcripts
    safe_date = document_date.isoformat() if document_date else "unknown"
    accession = f"transcript:{ticker}:{safe_date}:{fiscal_period or 'unknown'}"
...
def _is_already_ingested(conn: Any, ticker: str, document_date: date | None) -> bool:
    """Check if a transcript has already been processed (v2_documents record exists)."""
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            WHERE f.ticker = %(ticker)s
              AND f.document_date = %(document_date)s
              AND vd.status = 'complete'
            LIMIT 1
            """,
            {"ticker": ticker, "document_date": document_date},
        )
...
def _process_one(
    ...
    """Run the pipeline on one transcript and optionally persist results."""
    result = IngestResult(
        source_id=source_id,
        ticker=ticker,
    ...
    try:
        ...
        with db_adapter.transaction() as conn:
            company_id = _get_or_create_company(conn, ticker, company_name)
        ...
        persist_result = V2PersistenceAdapter(db_adapter).persist_pipeline_result(
            ...
            ticker=ticker,
            document_date=metadata.document_date,
            transcript_source=metadata.source,
        )
...
def run_ingestion(
    ...
    Args:
        source_name: "fmp" or "huggingface"
        tickers: List of ticker symbols to ingest
        limit: Max transcripts per ticker
    ...
    Returns:
        IngestSummary with per-transcript results
    """
    global _shutdown_requested

    if progress_file is None:
        progress_file = Path("logs/ingest_transcripts_progress.json")
    ...
    if source_name == "fmp":
        ...
        raise ValueError(f"Unknown source: {source_name!r}. Use 'fmp' or 'huggingface'.")
    ...
    # Build pipeline (transcript mode)
    ...
    for ticker in tickers:
        ...
        logger.info("Listing transcripts for %s from %s...", ticker, source_name)
        try:
            docs = source.list_available(ticker=ticker, limit=limit)
        except Exception as exc:
            logger.error("Failed to list transcripts for %s: %s", ticker, exc)
        ...
        logger.info("  Found %d transcript(s) for %s", len(docs), ticker)
        ticker_count = 0
        ...
        for meta in docs:
            ...
            if db_adapter and not dry_run and meta.document_date:
                with db_adapter.get_connection() as conn:
                    if _is_already_ingested(conn, ticker, meta.document_date):
                        logger.info(
                            "  Skipping %s %s — already ingested",
                            ticker,
                            meta.document_date,
                        )
                        summary.results.append(IngestResult(
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Batch ingest earnings call transcripts into the V2 pipeline.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__,
    )
    parser.add_argument(
        "--source",
        choices=["fmp", "huggingface"],
        default="fmp",
        help="Transcript source (default: fmp)",
    )
    parser.add_argument(
        "--tickers",
        nargs="+",
    ...
    parser.add_argument(
        ...
        default=5,
        metavar="N",
        help="Max transcripts per ticker (default: 5)",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def main() -> int:
    ...
    if not args.dry_run and not db_url:
        ...
        print("  Set DATABASE_URL or use --dry-run to skip database writes.", file=sys.stderr)
        return 1
