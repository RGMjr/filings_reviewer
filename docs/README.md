...
| `/learn [cleanup]` | Project-local: capture durable lessons from the current session into project memory, or audit/prune existing memory entries for staleness and redundancy. |
| `/metric-lifecycle` | Guidance for adding, deprecating, or removing metrics |
| `/monitor-prs` | Project-local: single-shot dynamic PR monitor — discovers the caller's open non-draft PRs and delegates to `/supervise-prs`. Compose with `/loop 8m /monitor-prs` to poll continuously. |
| `/pick-issues [count] [strategy]` | Project-local: select one or more known-issue fragments and draft dispatch-ready worker prompts. Strategies: `highest-impact` (default), `parallel-safe`, `xs-only`, `tier1-recall-gap`. |
...
| `/sweep` | Project-local: manually invoke the nightly KNOWN_ISSUES sweeper (same flow the Render cron runs). Useful for ad-hoc backlog drains and verifying sweeper changes. |
...
### v2.9 — 2026-08-13 — Post-pivot features, Sentry, analytics views, and doc maintenance

**New features since v2.8:**
- **Sentry error monitoring** wired into web, worker, and extraction services (#657)
- **Tier-1 disclosure analytics views** added for Metabase reporting — `v_analytics_tier1_*` (#618)
- **Simulate-accepted endpoint + migration** (`/api/v2/simulate-accepted`, `scripts/simulate_accepted.py`) for dry-running pattern promotions (#609)
- **Ship-to-PR machinery** for accepted text-pattern recommendations from `/v2/review/stats` — "Ship to PR" button and backend endpoint (#630, #638)
- **Phase-2 LLM presence classifier gate v2** — C3/C6 reframe, dedup, token-weighted aggregation; final verdict NO-GO, rollout permanently closed (#627, #632)
- **Simulation UI** on `/v2/review/stats` for previewing pattern impacts (#629)
- **`/cleanup` resolved-fragment GH sync** step auto-closes GitHub issues when a fragment is marked resolved (#639)
- **Image Confirmations stats counters** redefined to reflect per-metric review flow (#648)
- **Nightly sweeper**: fail-fast on expired GH_TOKEN (#622); section-classification heading-markup variant gap fixed (#624)

**`src/` directories added (omitted from v2.8 listing):**
- `src/auth/` — OAuth/session/CSRF/admin auth layer (Stage A–C rollout)
- `src/ml/` — image relevance model retrain runner

**Documentation fixes (this audit — 2026-08-13):**
- CLAUDE.md architecture listing updated to include `auth` and `ml`
- Slash commands table updated to include `/learn`, `/monitor-prs`, `/pick-issues`, `/sweep`
- 134 known-issue fragments remain at `status: resolved` and should be transitioned to `status: archived` (flagged; not auto-fixed)
