...
| `/learn [cleanup]` | Capture durable session lessons into project memory, or audit and prune stale/redundant memory entries. |
...
### v2.9 — 2026-09-10 — Post-pivot feature completions and monitoring

Work completed between v2.8 (2026-04-25) and this update:

- **Sentry error monitoring** (`feat` #657): Centralized init in `src/infra/sentry.py`; wired into Flask app factory, worker logging, and V2 extraction. Errors-only, PII-scrubbed, release-tagged. `SENTRY_DSN` added to `filings-shared-secrets` Render env group and documented in `docs/operations/setup-guide.md`.
- **Ship-to-PR machinery** (`feat` #630, #638): `simulate-accepted` endpoint + DB migration automates promoting accepted text-pattern recommendations into PRs. `/v2/review/stats` Patterns tab gained a Ship-to-PR button.
- **Simulation UI on `/v2/review/stats`** (`feat` #629): Interactive what-if view for evaluating extraction recommendation changes before shipping.
- **LLM presence classifier rollout closed** (`docs` #632): Option A adopted — classifier stays dormant (zero incremental recall on all 10 enrolled Tier-1 metrics across both live runs). Full closeout in `docs/analysis/llm-presence-classifier-rollout-closeout-20260515.md`.
- **Phase-2 gate improvements** (`feat` #627, `fix` #635): C3 reframe, dedup by filing-id, token-weighted cache rate (C6), and `selected_*` from post-dedup corpus.
- **Tier-1 analytics views** (`feat` #618): `v_analytics_tier1_disclosure_*` Postgres views for Metabase reporting.
- **Image Confirmations counters redefined** (`fix` #648): Stats counters now reflect per-metric review flow rather than per-image.
- **AR FP exclusion and financial row-count binding** (`fix` #647, #655): Reduces false positives on accounts-receivable context; tightens value binding for financial row patterns.
- **Vision-API provider guard** (`fix` #637): Prevents silent dark image pipeline when provider-specific env vars absent (gh-619).
- **Sweeper GH_TOKEN expiry fail-fast** (`fix` #622): Sweeper probes `gh api user` on startup and aborts with a clear error rather than silently failing mid-run (gh-620).
- **Cleanup skill GH-issue sync step** (`feat` #639): `/cleanup` now syncs resolved fragment statuses against live GitHub issue state.
- **CLAUDE.md + README (this file)**: Architecture listing updated to include `src/auth/` and `src/ml/`; slash commands table updated to include `/learn`, `/monitor-prs`, `/pick-issues`, and `/sweep`.
