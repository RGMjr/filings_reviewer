...
| Nightly sweeper uses GNU `timeout` — macOS incompatible (Issue #66) | Open | Local `/sweep` on Mac fails at the `timeout` call; Render (Linux) production is fine |
| `Dockerfile.nightly-sweep` installs `claude` + `gh` unpinned (Issue #67) | Open | Version drift between builds could silently change sweeper behaviour |
...
| #66   | safe     | XS        | `scripts/run_nightly_sweep.sh`                                          | Detect timeout vs gtimeout; fallback path for macOS           |
| #67   | review   | S         | `Dockerfile.nightly-sweep`                                              | Pin claude + gh versions; needs validation step               |
...
## 66. Nightly Sweeper Orchestrator Uses GNU `timeout` (Incompatible with macOS)
...
## 67. `Dockerfile.nightly-sweep` Installs `claude` + `gh` Unpinned
