...
| Issue | Status | Notes |
|-------|--------|-------|
| Robinhood Tier 1 GS regression vs. 2026-04-19 baseline (Issue #72) | Open | Chart pipeline stalled on HOOD S-1 — "Annual Revenue by Annual Cohort" image has null OCR / null chart_data, 0 facts; `$130` chart fact is an orphan referencing a vanished `img_id`. Pipeline-state drift, not a commit regression. Blocks PR merge-commits via pre-commit hook |
...
| #72   | skip     | M         | —                                                                       | Tier 1 regression; pipeline-state drift (chart stage didn't run), needs targeted re-extract + baseline refresh |
| #73   | skip     | S         | `tests/ui/*.spec.js tests/ui/test_server.py`                            | Playwright E2E gap — cross-filing auto-advance; needs stub-server extension |
| #74   | safe     | S         | `tests/integration/test_db_filings_reviewers.py`                        | New integration test for filings-list reviewer aggregate; isolated file |
...
`data/gold_standard/v2_baseline.json` was last refreshed at commit `cdc831f` (2026-04-19, Phase 3 chart bridge). Robinhood baseline: `recall=0.3143, f1=0.4231`. Current main state: `recall=0.171, f1=0.255` (-14pp recall on HOOD; -0.009 overall). Per-metric diagnostics:

| Metric | Tier | Current P/R/F1 | Notes |
|---|---|---|---|
| `cm_balance_by_cohort` | T1 | 0% / 0% / 0% | Same chart-pipeline failure mode as `cm_revenue_by_cohort` (both chart-only metrics on the same filing). Issue #64 — boundary sensitivity — is a separate, narrower concern already resolved |
| `cm_customer_acquisition_cost` | T1 | 100% / 50% / 66.7% | 1 FN — dedup collision (`20.0` collapsed into sibling with different value) |
| `cm_revenue_by_cohort` | T1 | 0% / 0% / 0% | 10 FNs, **chart pipeline stalled on source image** (details below) |

### Root cause: chart pipeline did not run on HOOD S-1 chart images

Investigation against the production DB (Neon, 2026-04-21) shows the drift is **pipeline-state**, not code. The "Annual Revenue by Annual Cohort ($mm)" image — the single source of all 10 gold-standard per-cohort values — exists in `v2_image_assets` (`img_id=e5f65961-f33f-44db-9fd0-5f3b61dae987`, `classification='chart'`, `relevance_score=0.66`) but has `processed=False`, `ocr_text IS NULL`, `chart_data IS NULL`, and zero linked facts. It was never processed by the chart stage.

Aggregate for HOOD's S-1 (`filing_id=1545`): 21 images, 17 classified as charts, **0 with `chart_data`, 0 with `ocr_text`**. Only 2 images are flagged `processed=True`, both with empty payloads.

The single `cm_revenue_by_cohort` chart fact in the DB — `value=130, value_raw='$130'` — references `img_id=c8da02f5-227c-4830-94d9-c45944d45e7f`, which **no longer exists in `v2_image_assets`**. It is an orphan fact from an earlier extraction run, stranded when img_ids were rekeyed (likely by `d94acab fix(extraction): dedup v2_image_assets + stabilise img_id across runs (#21)`).

The `$102,034.8` text-sourced fact the validator counts as an FP is an unrelated period-`2026-Q1` mis-bind from elsewhere in the S-1; it has nothing to do with cohort values.

### What the earlier PR-body diagnosis got wrong

- **Not a scale bug.** There is no `$33,421.5` anywhere in HOOD's facts. Extraction isn't binding a quarterly total in place of per-cohort values — it isn't binding the cohort chart at all.
- **Not caused by #52.** `24bfd6b` is a 3-file persistence refactor (`src/extraction_v2/persistence.py` + 2 tests); commit body reads "No behavioral change; 3630 tests pass." Zero value-binding or chart-stage code is touched.
- **"Only extraction-touching commit in the window" was wrong.** The `cdc831f..52e61ce` window includes `d94acab` (#21, img_id stabilisation), `9aeb454`/`cf0c756` (#34, image-cache → Cloudflare R2), `1d7c204` + `8cd3b4d` (#50, chart_only persistence mode). Any of these could plausibly have perturbed chart-stage orchestration or image retrievability.

### Why this landed on main

CI runs unit + integration tests but not the full v2 validator, so chart-stage coverage drift only surfaces when the pre-commit hook runs locally against a populated DB.

### Next Steps

1. **Re-run the chart stage for HOOD S-1** (`filing_id=1545`). The fastest diagnostic is a targeted re-extraction with `--force-reextract` (or equivalent) and then re-query the same image to see if `ocr_text` / `chart_data` populate. If they do, the regression is pipeline-state drift and the action is simply to re-populate + refresh the baseline.
2. **If re-extraction still fails to process the chart**, the code cause is likely in the image-cache migration (#34) or chart_only persistence mode (#50). Bisect on chart-stage behavior for HOOD with the debugger, not on validator recall (too noisy).
3. **Do NOT refresh the baseline until step 1 passes.** Refreshing over the stalled-pipeline state locks in zero cohort recall and removes the signal.
4. **Hole-closing follow-up** (separate issue, not blocking): add a CI job that runs the v2 validator against at least the Tier 1 metric subset so chart-stage coverage drift is caught pre-merge.
5. **Blocks**: PR #72 (Snap ingestion) cannot land a merge commit against current main while this regression is live — the same pre-commit hook fires on any merge commit.

---

## 73. Missing Playwright E2E for Cross-Filing Auto-Advance
...
## 74. Missing Integration Test for Filings-List Reviewer Aggregate
...
- **2026-04-21**: Added Issue #72 — Robinhood Tier 1 gold-standard regression vs. 2026-04-19 baseline surfaced during PR #72 (Snap ingestion) `git merge origin/main` attempt. Local pre-commit hook reported `recall_delta=-0.009, f1_delta=-0.0015, regressed_companies=['Robinhood Markets, Inc.']`. Investigation against Neon (2026-04-21) shows the drift is **pipeline-state, not code**: HOOD S-1 (`filing_id=1545`) has 17 chart-classified images, 0 with `ocr_text` and 0 with `chart_data`; the "Annual Revenue by Annual Cohort ($mm)" image (`img_id=e5f65961…`) is unprocessed and has 0 linked facts despite being the sole source of all 10 gold per-cohort values. The single `$130` chart fact in the DB references `img_id=c8da02f5…` which no longer exists in `v2_image_assets` — an orphan from a prior run (likely pre-`d94acab` img_id stabilisation). There is no `$33,421.5` anywhere in HOOD's facts; that figure from an earlier draft of this entry was incorrect. Commit `24bfd6b` (#52) is ruled out as root cause (persistence-only refactor, no extraction code). Real suspects in the `cdc831f..52e61ce` window are `d94acab` (#21, img_id dedup/stabilise), `9aeb454`/`cf0c756` (#34, image-cache to R2), and `1d7c204`/`8cd3b4d` (#50, chart_only persistence). Next action is a targeted re-run of the chart stage for HOOD S-1 before bisecting. Baseline must not be refreshed until the cohort chart actually produces `ocr_text`/`chart_data` again.
