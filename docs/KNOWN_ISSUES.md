...
| Issue | Status | Notes |
|-------|--------|-------|
| Robinhood Tier 1 GS regression vs. 2026-04-19 baseline (Issue #72) | Open | Chart pipeline stalled on HOOD S-1 — cohort image (`e5f65961…`) has null OCR/chart_data, 0 facts. Root cause: PR #34 added `boto3` to `requirements.txt` but not `pyproject.toml`/`uv.lock`, so `uv run` extraction crashed at stage 1. Fix in this PR unblocks ingestion; chart-image bytes missing from R2 is a second layer. Blocks PR merge-commits via pre-commit hook |
...
2. Open a separate issue for the R2 image-bytes layer. Decide whether to (a) backfill image bytes into R2 under the existing keys, (b) re-ingest HOOD S-1 from source HTML so ingestion re-uploads, or (c) reconcile the key-format divergence between code and CLAUDE.md/infrastructure.md docs.
